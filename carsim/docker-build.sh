...
# Direct adaptation of: https://github.com/eclipse/kuksa.val.services/blob/main/hvac_service/docker-build.sh
...
if [ "$DOCKER_ARCH" = "multiarch" ]; then
	...
	if [ $LOCAL -eq 1 ]; then
		...
		DOCKER_EXPORT="($DOCKER_ARCH/$DOCKER_IMAGE ${REGISTRY}/$DOCKER_IMAGE:prerelease)"
	else
...
DOCKER_BUILDKIT=1 docker buildx build --no-cache $DOCKER_ARGS -f Dockerfile "$CONTEXT_DIR" $DOCKER_EXT

if [ $? -eq 0 ]; then
	echo "docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ${REGISTRY}/$DOCKER_IMAGE:prerelease"
	docker image tag $DOCKER_ARCH/$DOCKER_IMAGE ${REGISTRY}/$DOCKER_IMAGE:prerelease
